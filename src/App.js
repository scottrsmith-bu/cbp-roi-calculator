...
function CBPROICalculator() {
  ...
  const filteredOrganizations = useMemo(() => {
    ...
    // filterType and sortBy kept for parity with UX; only name sort used here
    return filtered.sort((a, b) =>
      sortBy === 'name' ? a.name.localeCompare(b.name) : b.personnel - a.personnel
    );
  }, [searchTerm, sortBy, organizations]);
  ...
  const handleSendMessage = () => {
    ...
    setChatMessages(prev => ([
      ...
      { type: 'assistant', text: `Based on ${selectedOrganization?.name || 'CBP'}: ${chatInput}` }
    ]));
    setChatInput('');
  };
          ...
          <div style={{ padding: '32px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {filteredOrganizations.map((org) => (
                <div
                  key={org.id}
                  style={{ background: 'white', border: '2px solid #e5e7eb', borderRadius: '12px', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {showImpact && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {/* Performance Drivers with sliders */}
                    ...
                    <div key={d.key} style={{ border: '2px solid #dbeafe', borderRadius: '12px', padding: '20px', background: '#eff6ff' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                        ...
                        <div style={{ flex: 1, height: '40px', position: 'relative' }}>
                          <div style={{ position: 'absolute', width: '100%', height: '40px', background: '#93c5fd', borderRadius: '20px' }} />
                          <div style={{ position: 'absolute', height: '40px', background: '#1e40af', borderRadius: '20px', width: `${(d.growth / 70) * 100}%` }} />
                        </div>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#64748b', marginTop: '4px' }}>
                          <span>0% (Low)</span><span>15% (Medium)</span><span>30% (High)</span>
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      {showAssistant && (
        <div style={{ position: 'fixed', right: '24px', bottom: '24px', background: 'white', borderRadius: '12px', boxShadow: '0 8px 30px rgba(0,0,0,0.2)', border: '2px solid #0066cc', width: '384px', zIndex: 1000 }}>
          <div style={{ background: '#0066cc', color: 'white', padding: '16px', borderRadius: '10px 10px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => setShowAssistant(false)} style={{ background: 'transparent', border: 'none', color: 'white', cursor: 'pointer', padding: '4px', borderRadius: '4px' }}
              ...
              <div style={{ textAlign: 'center', paddingTop: '32px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {["How is the net savings calculated?", "Why is OFO facing a retirement crisis in 2028?", "Explain the dual-pathway model", "What are workers' comp claims?", "How do Performance Drivers affect ROI?"].map((q, i) => (
                    <button key={i} onClick={() => setChatInput(q)} style={{ width: '100%', textAlign: 'left', padding: '12px', background: 'white', border: '1px solid #e5e7eb', borderRadius: '6px', fontSize: '13px', cursor: 'pointer' }}
